...
echo "$($PSQL "
  SELECT name FROM games
    INNER JOIN teams ON winner_id = teams.team_id
    WHERE year='2018' AND round='Final'
")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "
  SELECT w.name AS team_name FROM games
    LEFT JOIN teams w ON winner_id = w.team_id
    WHERE year = '2014' AND round = 'Eighth-Final'

    UNION ALL

  SELECT o.name AS team_name FROM games
    LEFT JOIN teams o ON opponent_id = o.team_id
    WHERE year = '2014' AND round = 'Eighth-Final'
...
echo "$($PSQL "
  ...
  FROM (
    ...
    SELECT o.name AS team_name
    FROM games
    LEFT JOIN teams o ON opponent_id = o.team_id
    WHERE o.name LIKE 'Co%'
  ) AS combined_teams
  ORDER BY team_name; 
")"